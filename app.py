...
INVERTEXTO_TOKEN = '25199|bDgHJuJnPGDY2pDC8C7DZbSAjzoR2BYN'
...
def consulta_cep(cep_input):
    ...
    if len(cep_formatado) != 8:
        return jsonify({"Error": "CEP invalido! Deve conter 8 dígitos"}), 400
    ...
    try:
        # Cria o cursor e executa a consulta no BD
        ...
        #Verificar se o CEP existe no BD local
        if cep_bd_local:
            cursor.close()
            conn.close()
            dados = {
